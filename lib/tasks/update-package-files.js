...
const updatePackageFiles = (
  ...
  files.map((filePath) => {
    ...
    if (mfeType !== "shell" && filePath[1] !== "appRoutesComponentConfig.js") {
      return false;
    }

    if (mfeType !== "mfe-app" && filePath[1] === "appRoutesComponentConfig.js") {
      return false;
    }
    ...
    return fs.open(file, "r+").then((fd) =>
      fs
        ...
        .then((result) => {
          ...
          if (filePath[1] === "appRoutesComponentConfig.js") {
            return result.replace(
              /module_expose_[a-z]+/g,
              mfeAppProjectName.replace(/[-]+/g, "_")
            );
